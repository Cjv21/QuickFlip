...
namespace QuickFlip.BusinessLayer
{
    public class BusinessLogic
    {
        ...
        public static List<Post> GetRecommendationsByUserId(int userId)
        {
            ...
            List<Post> myPosts = GetPostsByUserId(userId);
            ...
            // posts i have bid on
            List<Post> postsIBidOn = GetPostsByBidderId(userId);
            List<Post> postsIWantToBuy = postsIBidOn.Where(x => x.PostType == PostType.Sell).ToList();
            List<Post> postsIWantToSell = postsIBidOn.Where(x => x.PostType == PostType.Buy).ToList();

            // GET POSTS THAT ARE LOOKING TO BUY WHAT I AM SELLING
            List<Post> wantToBuyMyStuff = new List<Post>();
            if (myForSale.Count != 0)
            {
                wantToBuyMyStuff = GetWantToBuyMyStuff(myForSale);

                recommendations.AddRange(wantToBuyMyStuff);
            }

            return recommendations;
        }

        public static List<Post> GetWantToBuyMyStuff(List<Post> myForSale)
        {
            User me = GetUserByUserId(myForSale.First().UserId);

            List<Post> buyPosts = GetAllPosts().Where(x => x.PostType == PostType.Buy).ToList();

            // remove posts that aren't in my community and are not willing to ship
            buyPosts.RemoveAll(x => 
                x.UserId == me.UserId || 
                (x.CommunityId != me.CommunityId && 
            ...
            {
                wantToBuyMyStuff.AddRange(GetSimilarPosts(post, PostType.Buy));
            }

            return wantToBuyMyStuff;
        }
        
        public static List<Post> GetSimilarPosts(Post postToBeSimilarTo, PostType type)
        {
            List<Post> posts = GetAllPosts();
            posts.RemoveAll(x => (x.CommunityId != postToBeSimilarTo.CommunityId && x.TransactionType == TransactionType.Local));
            posts.RemoveAll(x => x.PostType != type);
