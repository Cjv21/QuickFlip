...
namespace QuickFlip.BusinessLayer
{
    public class BusinessLogic
    {
        ...
        public static List<Post> GetRecommendationsByUserId(int userId)
        {
            ...
            {
                wantToBuyMyStuff = GetWantToBuyMyStuff(myForSale, posts);
        ...
        public static List<Post> GetWantToBuyMyStuff(List<Post> myForSale, List<Post> posts)
        {
            User me = GetUserByUserId(myForSale.First().UserId);

            List<Post> buyPosts = posts.Where(x => x.PostType == PostType.Buy).ToList();
            ...
            {
                wantToBuyMyStuff.AddRange(GetSimilarPosts(post, PostType.Buy, posts));
            }
        ...
        {
            List<Post> validPosts = posts.Where(x =>
                x.PostType == type &&
                x.UserId != postToBeSimilarTo.UserId && 
                (x.CommunityId == postToBeSimilarTo.CommunityId ||
                 (x.CommunityId != postToBeSimilarTo.CommunityId &&
                  x.TransactionType == TransactionType.LocalOrLongDistance))
            ).ToList();
        ...
        public static List<string> GetKeywords(Post post)
        {
            ...
            return keywordCount.OrderByDescending(k => k.Count).ThenByDescending(k => k.ToString().Length)
                .Select(k => k.Keyword).Take(3).ToList();
        }
